...
Open items (not yet fixed): capability enforcement at the syscall boundary (#3), USB HID unification, per-socket TCP state. Kernel-linked services still use a cloned 4 GiB identity map at CPL0. SCI guests use `domain-create-user`: exclusive maps (trampoline, published ISR page, IDT, TSS stack, and U=1 4 KiB image/stub/stack pages). Timer/syscall/exception stubs switch to `kernel_cr3`. No 4 MiB or 4 GiB identity clone. Guests enter CPL3 via `domain-enter-user`.
