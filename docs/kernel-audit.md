...
Open items (not yet fixed): capability enforcement at the syscall boundary (#3), USB HID unification, per-socket TCP state. Kernel-linked services still use a cloned 4 GiB identity map at CPL0. SCI guests use `domain-create-user`: supervisor-only 4 MiB identity (trampoline/GDT/TSS/kernel image/globals), exclusive U=1 4 KiB pages for the image/stub/stack in the 1 GiB window, and no 2–4 GiB clone. Guests enter CPL3 via `domain-enter-user`.

Fixed later (2026-09-16): DNS compression pointers are followed and must point strictly earlier; SparkFS caps `total`/`ino-count` and rejects out-of-range `bno`; PCI BAR decode rejects I/O/zero/undersized MMIO; TCP ISN is mixed from `ticks` and `sock-close` sends FIN+ACK; UDP payload copy is capped at 1472 and `sock-sendto` puts `rip`/`rport` on the wire; `posix-elf-exec-image` copies PT_LOAD and zeroes BSS (still a CPL0 `invoke1`); `proc-wait` rejects negative pids. GDT has DPL3 user segments and a TSS; `enter_user`/`leave_user` iretq to CPL3; SCI guests (`sci-load`, `sci-load-file`) return via `int 0x80` sys_exit. Hardening asserts a CPL3 `mov rax,0x42; ret` round-trip, guest kernel-globals U=0, and no 2–4 GiB identity clone.
