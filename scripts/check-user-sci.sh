#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUILD_DIR="${BUILD_DIR:-/tmp/space-user-sci-check}"
SERIAL="$BUILD_DIR/serial.log"
FIFO="$BUILD_DIR/serial_in"

BUILD_DIR="$BUILD_DIR" "$SCRIPT_DIR/build-user-sci.sh"
rm -f "$SERIAL" "$FIFO"
mkfifo "$FIFO"
qemu-system-x86_64 -kernel "$BUILD_DIR/combined.bin" -m 256M \
  -device isa-debug-exit,iobase=0xf4 -display none -serial stdio -no-reboot \
  <"$FIFO" >"$SERIAL" 2>/dev/null &
QPID=$!
exec 3>"$FIFO"
for _ in $(seq 1 400); do
  grep -qF "interactive shell" "$SERIAL" 2>/dev/null && break
  kill -0 "$QPID" 2>/dev/null || break
  sleep 0.1
done
if ! grep -qF "interactive shell" "$SERIAL" 2>/dev/null; then
  echo "FAIL: shell did not start" >&2
  kill "$QPID" 2>/dev/null || true
  wait "$QPID" 2>/dev/null || true
  rm -f "$FIFO"
  exit 1
fi

echo "hello" >&3
for _ in $(seq 1 200); do
  if grep -qF "hello from user SCI" "$SERIAL" 2>/dev/null \
     || grep -qF "SCI: component returned status 0x0000000000004849" "$SERIAL" 2>/dev/null; then
    break
  fi
  kill -0 "$QPID" 2>/dev/null || break
  sleep 0.1
done

echo "uecho" >&3
for _ in $(seq 1 200); do
  if grep -qF "user echo SCI" "$SERIAL" 2>/dev/null \
     || grep -qF "SCI: component returned status 0x0000000000004543" "$SERIAL" 2>/dev/null; then
    break
  fi
  kill -0 "$QPID" 2>/dev/null || break
  sleep 0.1
done

echo "halt" >&3
exec 3>&-
sleep 0.5
kill "$QPID" 2>/dev/null || true
wait "$QPID" 2>/dev/null || true
rm -f "$FIFO"

ok_hello=0
ok_echo=0
if grep -qF "hello from user SCI" "$SERIAL" 2>/dev/null \
   || grep -qF "SCI: component returned status 0x0000000000004849" "$SERIAL" 2>/dev/null; then
  ok_hello=1
fi
if grep -qF "user echo SCI" "$SERIAL" 2>/dev/null \
   || grep -qF "SCI: component returned status 0x0000000000004543" "$SERIAL" 2>/dev/null; then
  ok_echo=1
fi
if [ "$ok_hello" != "1" ]; then
  echo "FAIL: user SCI hello marker missing" >&2
  sed -n '/space>/,/halt/p' "$SERIAL" 2>/dev/null | tail -40 >&2 || true
  exit 1
fi
if [ "$ok_echo" != "1" ]; then
  echo "FAIL: user SCI uecho marker missing" >&2
  sed -n '/space>/,/halt/p' "$SERIAL" 2>/dev/null | tail -40 >&2 || true
  exit 1
fi
echo "PASS: user SCI hello + uecho loaded and ran"
